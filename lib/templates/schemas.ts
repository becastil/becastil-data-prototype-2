...
export const HealthcareCostRowSchema = z.enum([
...
export const MonthColumnSchema = z.enum([
  'Jan-2024', 'Feb-2024', 'Mar-2024', 'Apr-2024', 'May-2024', 'Jun-2024',
  'Jul-2024', 'Aug-2024', 'Sep-2024', 'Oct-2024', 'Nov-2024', 'Dec-2024'
])

export const HealthcareCostDataSchema = z.object({
  Category: HealthcareCostRowSchema,
  'Jan-2024': z.number().nullable(),
  'Feb-2024': z.number().nullable(),
  'Mar-2024': z.number().nullable(),
  'Apr-2024': z.number().nullable(),
  'May-2024': z.number().nullable(),
  'Jun-2024': z.number().nullable(),
  'Jul-2024': z.number().nullable(),
  'Aug-2024': z.number().nullable(),
  'Sep-2024': z.number().nullable(),
  'Oct-2024': z.number().nullable(),
  'Nov-2024': z.number().nullable(),
  'Dec-2024': z.number().nullable()
})

// High Cost Claims Template Schema
export const MemberTypeSchema = z.enum(['Subscriber', 'Spouse', 'Dependent'])
export const AgeBandSchema = z.enum(['<1 - 19', '20 - 29', '30 - 39', '40 - 49', '50 - 59', '60 - 64', '65+'])
export const EnrolledSchema = z.enum(['Yes', 'No'])
export const HitStopLossSchema = z.enum(['Yes', 'No'])

export const HighCostClaimsDataSchema = z.object({
  'Member ID': z.number().int().positive(),
  'Member Type': MemberTypeSchema,
  'Age Band': AgeBandSchema,
  'Primary Diagnosis Category': z.string().min(1),
  'Specific Diagnosis Details Short': z.string().min(1),
  'Specific Diagnosis Details': z.string().min(1),
  '% of Plan Paid': z.number().min(0).max(1),
  '% of large claims': z.number().min(0).max(1),
  'Total': z.number().min(0), // This will be calculated
  'Facility Inpatient': z.number().min(0),
  'Facility Outpatient': z.number().min(0),
  'Professional': z.number().min(0),
  'Pharmacy': z.number().min(0),
  'Top Provider': z.string().min(1),
  'Enrolled': EnrolledSchema,
  'Stop-Loss Deductible': z.number().min(0),
  'Estimated Stop-Loss Reimbursement': z.number().min(0), // This will be calculated
  'Hit Stop Loss?': HitStopLossSchema
})

// Template validation schemas
export const HealthcareCostTemplateSchema = z.array(HealthcareCostDataSchema)
export const HighCostClaimsTemplateSchema = z.array(HighCostClaimsDataSchema)

// Expected row order for Healthcare Cost template
export const HEALTHCARE_COST_ROW_ORDER = [
  'MEDICAL CLAIMS',
  'Domestic Medical Facility Claims (Inpatient/Outpatient)',
  'Non-Domestic Medical Claims (Inpatient/Outpatient)',
  'Total Hospital Medical Claims',
...
export const HIGH_COST_CLAIMS_COLUMN_ORDER = [
  'Member ID',
  'Member Type',
  'Age Band',
...
export const FORMULA_ROW_INDICES = {
...
export type MonthColumn = z.infer<typeof MonthColumnSchema>
export type HealthcareCostData = z.infer<typeof HealthcareCostDataSchema>
export type HighCostClaimsData = z.infer<typeof HighCostClaimsDataSchema>
export type MemberType = z.infer<typeof MemberTypeSchema>
export type AgeBand = z.infer<typeof AgeBandSchema>
export type Enrolled = z.infer<typeof EnrolledSchema>
export type HitStopLoss = z.infer<typeof HitStopLossSchema>